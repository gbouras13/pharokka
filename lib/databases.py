...
def instantiate_install(db_dir):
    ...
    if downloaded_flag == True:
        ...
        if(md5_sum == requiredmd5):
            ...
            logger.error(f"Error: corrupt database file! MD5 should be '{requiredmd5}' but is '{md5_sum}'")
...
def download(db_url: str, tarball_path: Path):
    try:
        ...
        logger.error(f'ERROR: Could not download file from Zenodo! url={db_url}, path={tarball_path}')
...
def untar(tarball_path: Path, output_path: Path):
    try:
        ...
        files_to_move = [f for f in os.listdir(tarpath) if os.path.isfile(os.path.join(tarpath, f))]
