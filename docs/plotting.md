...
Pharokka v1.3.0 implements `pharokka plot`, which creates a simple circular genome plot using [pyCirclize](https://github.com/moshi4/pyCirclize) with output in PNG format. All CDS are coloured according to their PHROG functional group. 
...
You can run `pharokka plot` in the following way (most basic command below).
...
`pharokka plot -i input.fasta -n pharokka_plot -o pharokka_output_directory -p my_prefix`

If you want to give your plot a title (e.g. phage lambda): 

`pharokka plot -i input.fasta -n pharokka_plot -o pharokka_output_directory -t 'phage lambda' `

To add italics (especially for genus or species names), encase them with `${ }$` as follows: 

`pharokka plot -i input.fasta -n pharokka_plot -o pharokka_output_directory -t '${Escherichia}$ phage lambda' `

If you want to make the title bigger:

`pharokka plot -i input.fasta -n pharokka_plot -o pharokka_output_directory -t '${Escherichia}$ phage lambda' --title_size 30 `
...
`pharokka plot -i input.fasta -n pharokka_plot -o pharokka_output_directory --annotations 0.5 `

You can also use this to remove all CDS labels by specifying `--annotations 0`

`pharokka plot -i input.fasta -n pharokka_plot -o pharokka_output_directory --annotations 0 `

By default hypothetical or unknown genes are not labelled. If you want to label them use `--label_hypotheticals`:

`pharokka plot -i input.fasta -n pharokka_plot -o pharokka_output_directory --label_hypotheticals `

If you want the axis intervals to be changed (e.g. 10kbp here):

`pharokka plot -i input.fasta -n pharokka_plot -o pharokka_output_directory -t '${Escherichia}$ phage lambda' --title_size 30 --interval 10000 `
...
`pharokka plot -i input.fasta -n pharokka_plot -o pharokka_output_directory --truncate 15`

If you want to make the CDS labels bigger (defaults to 8):

`pharokka plot -i input.fasta -n pharokka_plot -o pharokka_output_directory --label_size 10`

If you want to change the plot resolution (default 600 dpi)

`pharokka plot -i input.fasta -n pharokka_plot -o pharokka_output_directory -dpi 600`
...
`pharokka plot -i input.fasta -n pharokka_plot --gff pharokka.gff --genbank pharokka.gbk`
...
`pharokka plot -i input.fasta -n pharokka_plot -o pharokka_output_directory --remove_other_features_labels`
...
`pharokka plot -i input.fasta -n pharokka_plot -o pharokka_output_directory --label_ids labels.txt`



```bash
usage: pharokka plot [-h] -i INFILE [-n PLOT_NAME] [-o OUTDIR] [--gff GFF] [--genbank GENBANK] [-p PREFIX] [-t PLOT_TITLE] [-f]
...
pharokka plot: pharokka plotting function
...
options:
  ...
  --interval INTERVAL   Axis tick interval. Must be an integer. Defaults to 5000.
  ...
  --dpi DPI             Resolution (dots per inch). Must be an integer. Defaults to 600.
  --annotations ANNOTATIONS
                        Controls the proportion of annotations labelled. Must be a number between 0 and 1 inclusive. 
...
After receiving a number of requests, `pharokka` v1.7.0 implements `pharokka multiplot` that can be used to plot multiple contigs.
...
The rest of the parameters are the same as `pharokka plot`

```bash
usage: pharokka multiplot [-h] -g GENBANK -o OUTDIR [-f] [--label_hypotheticals] [--remove_other_features_labels]
...
pharokka multiplot: pharokka plotting function for muliple phages
