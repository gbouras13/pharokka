...
def test_overall_mash_distance(tmp_dir):
    ...
    input_fasta: Path = f"{standard_data}/SAOMS1.fasta"
    cmd = f"pharokka run -i {input_fasta} -d {database_dir} -o {tmp_dir} -t {threads} -f --mash_distance 0.05"
    exec_command(cmd)
...
def test_overall_crispr_minced_args(tmp_dir):
    ...
    input_fasta: Path = f"{CRISPR_data}/Biggiephage_A_fullcontig_CasΦ1.fasta"
    cmd = f'pharokka run -i {input_fasta} -d {database_dir} -o {tmp_dir} -t {threads} -f -g prodigal --minced_args "minNR 2 -minRL 21" '
    exec_command(cmd)
...
def test_overall_vfdb(tmp_dir):
    ...
    cmd = f"pharokka run -i {input_fasta} -d {database_dir} -o {tmp_dir} -t {threads} -f --skip_extra_annotations"
    exec_command(cmd)
...
def test_overall_amr(tmp_dir):
    ...
    cmd = f"pharokka run -i {input_fasta} -d {database_dir} -o {tmp_dir} -t {threads} -f --skip_mash"
    exec_command(cmd)
...
def test_overall_numeric_header_prodigal(tmp_dir):
    ...
    cmd = f"pharokka run -i {input_fasta} -d {database_dir} -o {tmp_dir} -t {threads} -f -g prodigal --fast -m"
    exec_command(cmd)
...
def test_overall_numeric_header_prodigal_gv(tmp_dir):
    ...
    input_fasta: Path = f"{standard_data}/SAOMS1_numeric_header.fasta"
    cmd = f"pharokka run -i {input_fasta} -d {database_dir} -o {tmp_dir} -t {threads} -f -m --fast"
    exec_command(cmd)
...
def test_meta(tmp_dir):
    ...
    cmd = f"pharokka run -i {input_fasta} -d {database_dir} -o {tmp_dir} -t {threads} -f -m"
    exec_command(cmd)
...
def test_pyrodigal_gv(tmp_dir):
    ...
    input_fasta: Path = f"{meta_data}/combined_meta.fasta"
    cmd = f"pharokka run -i {input_fasta} -d {database_dir} -o {tmp_dir} -t {threads} -f -m -g prodigal-gv"
    exec_command(cmd)
...
def test_prodigal_rv(tmp_dir):
    ...
    cmd = f"pharokka run -i {input_fasta} -d {database_dir} -o {tmp_dir} -t {threads} -f -g pyrodigal-rv"
    exec_command(cmd)
...
def test_meta_dnaapler_all_bug(tmp_dir):
    ...
    input_fasta: Path = f"{meta_data}/combined_meta.fasta"
    cmd = f"pharokka run -i {input_fasta} -d {database_dir} -o {tmp_dir} -t {threads} -f -m -s --dnaapler --meta_hmm"
    exec_command(cmd)
...
def test_overall_locus(tmp_dir):
    ...
    input_fasta: Path = f"{standard_data}/SAOMS1.fasta"
    cmd = f"pharokka run -i {input_fasta} -d {database_dir} -o {tmp_dir} -t {threads} -f -l SAOMS1 -p SAOMS1"
    exec_command(cmd)
...
def test_custom(tmp_dir):
    ...
    cmd = f"pharokka run -i {input_fasta} -d {database_dir} --custom_hmm {custom_db} -o {tmp_dir} -t {threads} -f "
    exec_command(cmd)
...
def test_custom_meta(tmp_dir):
    ...
    cmd = f"pharokka run -i {input_fasta} -d {database_dir} --custom_hmm {custom_db} -o {tmp_dir} -t {threads} -f -m"
    exec_command(cmd)
...
def test_overall_prodigal(tmp_dir):
    ...
    input_fasta: Path = f"{standard_data}/SAOMS1.fasta"
    cmd = f"pharokka run -i {input_fasta} -d {database_dir} -o {tmp_dir} -t {threads} -f -g prodigal"
    exec_command(cmd)
...
def test_overall_stop_recode(tmp_dir):
    ...
    cmd = f"pharokka run -i {input_fasta} -d {database_dir} -o {tmp_dir} -t {threads} -f -g prodigal -c 4"
    exec_command(cmd)
...
def test_overall_dnaapler(tmp_dir):
    ...
    input_fasta: Path = f"{standard_data}/SAOMS1.fasta"
    cmd = f"pharokka run -i {input_fasta} -d {database_dir} -o {tmp_dir} -t {threads} -f --dnaapler"
    exec_command(cmd)
...
def test_overall_mmseqs(tmp_dir):
    ...
    input_fasta: Path = f"{standard_data}/SAOMS1.fasta"
    cmd = f"pharokka run -i {input_fasta} -d {database_dir} -o {tmp_dir} -t {threads} -f --mmseqs2_only"
    exec_command(cmd)
...
#     input_fasta: Path = f"{meta_data}/fake_meta.fa"
#     cmd = f"pharokka run -i {input_fasta} -d {database_dir} -o {tmp_dir} -t {threads} -f -m --meta_hmm"
#     exec_command(cmd)
...
#     input_fasta: Path = f"{meta_data}/fake_meta.fa"
#     cmd = f"pharokka run -i {input_fasta} -d {database_dir} -o {tmp_dir} -t {threads} -f -m -s"
#     exec_command(cmd)
...
def test_terminase(tmp_dir):
    ...
    input_fasta: Path = f"{standard_data}/SAOMS1.fasta"
    cmd = f"pharokka run -i {input_fasta} -d {database_dir} -o {tmp_dir} -t {threads} -f --terminase --terminase_start 340 --terminase_strand neg"
    exec_command(cmd)
...
def test_overall_genbank(tmp_dir):
    ...
    cmd = f"pharokka run -i {input_gbk} -d {database_dir} -o {tmp_dir} -t {threads} -f --genbank"
    exec_command(cmd)
...
def test_overall_genbank_meta(tmp_dir):
    ...
    cmd = f"pharokka run -i {input_gbk} -d {database_dir} -o {tmp_dir} -t {threads} -f --genbank -m --meta_hmm"
    exec_command(cmd)
...
class testFails(unittest.TestCase):
    ...
    def test_dupe_header(self):
        ...
        with self.assertRaises(RuntimeError):
            ...
            cmd = f"pharokka run -i {input_fasta} -d {database_dir} -o {temp_dir} -t 1 -f -m"
            exec_command(cmd)
    ...
    def test_overall_hash_header(self):
        ...
        with self.assertRaises(RuntimeError):
            ...
            cmd = f"pharokka run -i {input_fasta} -d {database_dir} -o {temp_dir} -t {threads} -f"
            exec_command(cmd)
    ...
    def test_meta_terminas(self):
        ...
        with self.assertRaises(RuntimeError):
            input_fasta: Path = f"{meta_data}/fake_meta.fa"
            cmd = f"pharokka run -i {input_fasta} -d {database_dir} -o {temp_dir} -t 1 -f -m --terminase"
            exec_command(cmd)
    ...
    def test_bad_terminase(self):
        ...
        with self.assertRaises(RuntimeError):
            input_fasta: Path = f"{standard_data}/SAOMS1.fasta"
            cmd = f"pharokka run -i {input_fasta} -d {database_dir} -o {temp_dir} -t 1 -f --terminase"
            exec_command(cmd)
    ...
    def test_bad_terminase_start(self):
        ...
        with self.assertRaises(RuntimeError):
            input_fasta: Path = f"{standard_data}/SAOMS1.fasta"
            cmd = f"pharokka run -i {input_fasta} -d {database_dir} -o {temp_dir} -t 1 -f --terminase --terminase_start sf"
            exec_command(cmd)
    ...
    def test_bad_terminase_strand(self):
        ...
        with self.assertRaises(RuntimeError):
            input_fasta: Path = f"{standard_data}/SAOMS1.fasta"
            cmd = f"pharokka run -i {input_fasta} -d {database_dir} -o {temp_dir} -t 1 -f --terminase --terminase_start 34 --terminase_strand posit"
            exec_command(cmd)
    ...
    def test_bad_fast_meta_hmm(self):
        ...
        with self.assertRaises(RuntimeError):
            input_fasta: Path = f"{standard_data}/SAOMS1.fasta"
            cmd = f"pharokka run -i {input_fasta} -d {database_dir} -o {temp_dir} -t {threads} --fast --meta_hmm -m -f"
            exec_command(cmd)
    ...
    def test_bad_fast_meta_gmm(self):
        ...
        with self.assertRaises(RuntimeError):
            input_fasta: Path = f"{standard_data}/SAOMS1.fasta"
            cmd = f"pharokka run -i {input_fasta} -d {database_dir} -o {temp_dir} -t {threads} --mmseqs2_only --meta_hmm -m -f"
            exec_command(cmd)
    ...
    def test_bad_gene_pred(self):
        ...
        with self.assertRaises(RuntimeError):
            input_fasta: Path = f"{standard_data}/SAOMS1.fasta"
            cmd = f"pharokka run -i {input_fasta} -d {database_dir} -o {temp_dir} -t 1 -g proddigal -f"
            exec_command(cmd)
    ...
    def test_bad_genbank(self):
        ...
        with self.assertRaises(RuntimeError):
            ...
            cmd = f"pharokka run -i {input_gbk} -d {database_dir} -o {temp_dir} -t {threads} -f "
            exec_command(cmd)
    ...
    def test_custom_bad(self):
        ...
        with self.assertRaises(RuntimeError):
            ...
            input_fasta: Path = f"{custom_data}/MH649026.fasta"
            cmd = f"pharokka run -i {input_fasta} -d {database_dir} --custom_hmm {custom_db} -o {temp_dir} -t {threads} -f "
            exec_command(cmd)
