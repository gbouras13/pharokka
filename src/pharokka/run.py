"""
Entry point for `pharokka run` – annotate phage genome(s).

Adapted from bin/pharokka.py with relative package imports.
"""
...
from .databases import check_db_installation, check_rfam_installation
...
from .rfam import load_rfam_metadata, parse_cmscan_tblout, write_ncrna_tsv
...
def main():
    ...
    if args.dnaapler is False:
        ...
        if args.terminase_strand != "nothing" and args.terminase is False:
            logger.info(
        ...
        if args.terminase_strand != "nothing" and args.terminase_start == "nothing":
            logger.info(
    ...
    if args.rfam is True:
        ...
        ncrna_df = parse_cmscan_tblout(
            os.path.join(out_dir, prefix + "_cmscan.tblout"),
            metadata=load_rfam_metadata(db_dir),
            keep_trna=args.rfam_keep_trna,
        )
    ...
    # adds the merged df, vfdb and card top hits dfs to the class object
    ...
    if args.split is True and args.meta is True:
