"""
Entry point for `pharokka plot` – plot a single phage genome.

Adapted from bin/pharokka_plotter.py with relative package imports.
"""
...
def get_input():
    """Gets input for pharokka plot.
    :return: args
    """
    parser = argparse.ArgumentParser(
        description="pharokka plot: pharokka plotting function",
    ...
    parser.add_argument(
        ...
        help='Output plot file name. ".png" suffix will be added to this automatically. \nWill be output in the Pharokka output directory if -o is specified, or in the working directory if --gff and --genbank are specified.',
    )
    parser.add_argument(
...
def main():
    ...
    logger.info("Running pharokka plot to plot your phage.")
    ...
    if args.force is True:
        if os.path.isfile(plot_file) is True:
            os.remove(plot_file)
        else:
            logger.warning(
    ...
    if args.force is True:
        if os.path.isfile(svg_plot_file) is True:
    ...
    # gff file
    if gff_genbank_flag is True:
        gff_file = args.gff
    else:
        gff_file = os.path.join(args.outdir, args.prefix + ".gff")

    if os.path.isfile(gff_file) is False:
        logger.error(
    ...
    # validate gbk exists
    if os.path.isfile(gbk_file) is False:
        logger.error(
    ...
    if args.label_ids != "":
        ...
        if os.path.isfile(args.label_ids) is False:
        ...
        try:
            ...
            logger.warning(
                f"{args.label_ids} contains no text. No contigs will be ignored"
            )
